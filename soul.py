...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*JISAI ES BOT PAI ACCESS CHAHEYE VO MUJAI DM KRO :-@SHARP_X_OWNER*", parse_mode='Markdown')
...
def attack_command(message):
    ...
    bot.send_message(chat_id, "*MOTHER FUCKER JALDI IP PORT TIME DAL:*\n* <host> <port> <time>*", parse_mode='Markdown')
...
def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            bot.send_message(message.chat.id, "*START KR MADEECHOD /start*", parse_mode='Markdown')
            return
        ...
        bot.send_message(message.chat.id, f"*🚀 GAND MARNA START HOGYA HAI YHA KYA DAIKH RHA HAI GAME MAI JA BSDK! 💥\n\n🗺️ Target IP: {target_ip}\n🔌 Target Port: {target_port}\n⏳ Duration: {duration} seconds*", parse_mode='Markdown')
...
def send_welcome(message):
    ...
    btn5 = KeyboardButton("JISAI ADMIN LAINA HAI YHA CLICK KRO")
    btn6 = KeyboardButton("")

    # Add buttons to the markup
    markup.add(btn1, btn2, btn3, btn4, btn5, btn6)

    bot.send_message(message.chat.id, "*🚀LUND READY HAI MARNE KAI LIY🚀*", reply_markup=markup, parse_mode='Markdown')
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
    ...
    elif message.text == "JISAI ADMIN LAINA HAI YHA CLICK KRO":
        ...
        if user_data:
            ...
            response = "*BAITA TAIRE BAAP SAI ACCESS LAI FIR ATTACK LGAIGA.*"
        
        bot.reply_to(message, response, parse_mode='Markdown')
    elif message.text == "🤖STRESSER SERVER":
        bot.reply_to(message, "*🤖BGMI KE MAA CHUDNA START HO GYA HAI SAB MARO....*", parse_mode='Markdown')
...
if __name__ == "__main__":
    ...
    logging.info(" BGMI KE MAA CHUDNA START HO GYA HAI SAB MARO.....")
    while True:
        try:
